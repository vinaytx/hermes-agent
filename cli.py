...
class HermesCLI:
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            self._handle_trace_command(cmd_original)
        elif canonical == "yolo":
            self._toggle_yolo()
    ...
    def _handle_trace_command(self, cmd: str):
        """Handle /trace — toggle call-flow tracing at runtime.

        Usage:
            /trace              Show current tracing status
            /trace status       Show current tracing status
        ...
        # parts[0] is "/trace", so subcommand is parts[1] if present
        ...
        if sub in ("on", "1", "enable"):
            ...
            from datetime import datetime
            log_path = get_hermes_home() / "logs" / f"trace_{datetime.now().strftime('%Y-%m-%d')}.log"
            _cprint(
                f"  Tracing {_Colors.BOLD}{_Colors.GREEN}ON{_Colors.RESET}"
                f" — writing to {log_path}"
            )

        elif sub in ("off", "0", "disable"):
            os.environ.pop("HERMES_TRACE", None)
            _cprint(f"  Tracing {_Colors.BOLD}{_Colors.RED}OFF{_Colors.RESET}")

        elif sub == "system":
            if sub2 in ("on", "1", "enable"):
                os.environ["HERMES_TRACE_SYSTEM_PROMPT"] = "1"
                _cprint(
                    f"  System prompt in trace: {_Colors.BOLD}{_Colors.GREEN}ON{_Colors.RESET}"
                    " — system message will be expanded in each LLM send block"
                )
            elif sub2 in ("off", "0", "disable"):
                os.environ.pop("HERMES_TRACE_SYSTEM_PROMPT", None)
                _cprint(
                    f"  System prompt in trace: {_Colors.BOLD}{_Colors.YELLOW}OFF{_Colors.RESET}"
                    " — system message shown as summary line only"
                )
            else:
                toggle = "1" if not sys_on else None
                if toggle:
                    os.environ["HERMES_TRACE_SYSTEM_PROMPT"] = toggle
                else:
                    os.environ.pop("HERMES_TRACE_SYSTEM_PROMPT", None)
                new_sys = not sys_on
                state = (f"{_Colors.GREEN}ON{_Colors.RESET}" if new_sys
                         else f"{_Colors.YELLOW}OFF{_Colors.RESET}")
                _cprint(f"  System prompt in trace: {_Colors.BOLD}{state}")

        else:  # status (default)
            trace_state = (f"{_Colors.GREEN}ON{_Colors.RESET}" if trace_on
                           else f"{_Colors.RED}OFF{_Colors.RESET}")
            sys_state = (f"{_Colors.GREEN}ON{_Colors.RESET}" if sys_on
                         else f"{_Colors.YELLOW}OFF (summary only){_Colors.RESET}")
            from hermes_constants import get_hermes_home
            from datetime import datetime
            log_path = get_hermes_home() / "logs" / f"trace_{datetime.now().strftime('%Y-%m-%d')}.log"
            _cprint(
                f"  Tracing:       {_Colors.BOLD}{trace_state}\n"
                f"  System prompt: {_Colors.BOLD}{sys_state}\n"
                f"  Log file:      {log_path}\n"
                f"\n"
